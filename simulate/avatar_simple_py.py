...
class AvatarDrakeEnv:
  def __init__(self, timestep, hand_controller_type, hand_state, arm_controller_type, motion, teleop_type, debug, plot):
    ...
    self.hand_kp = []
    self.hand_kd = []
    self.panda_model_urdf = "data/avatar/urdf/panda_arm.urdf"
    # self.hand_model_urdf  = "data/avatar/urdf/avatar_gripper_right_nomimic.urdf"
    self.hand_model_urdf = "data/avatar/urdf/avatar_gripper_right.urdf"
    self.table_top_sdf = "data/avatar/env_objects/table_top.sdf"
    self.right_arm_joint_names_indices = {}
    self.left_arm_joint_names_indices = {}
  ...
  def create_scene(self):
    ...
    container_instance = self.parser.AddModelFromFile("data/avatar/env_objects/container.sdf")
    self.plant.WeldFrames(
      self.plant.GetFrameByName("table_top_center"),
      self.plant.GetFrameByName("container_center"),
      RigidTransform(RollPitchYaw(0,0,0), [0.0, 0.3, 0.01])
    )
    cylinder_instance = self.parser.AddModelFromFile("data/avatar/env_objects/cylinder.sdf")
    cube_instance = self.parser.AddModels("data/avatar/env_objects/cube.sdf")
  ...
  def init_joints(self):
    ...
    index = 0
    self.plant.Finalize()
    dofs = articulated_model_instance_dofset(self.plant, self.hand_instance)
    print("dofs.v: ", dofs.v.shape)
    print("dofs.q: ", dofs.q.shape)
    print("dofs.u: ", dofs.u.shape)
    a = self.plant.CreateDefaultContext()
    M = self.plant.CalcMassMatrixViaInverseDynamics(a)
    ...
    # for joint_index in self.plant.GetJointIndices(self.panda_instance):
      ...
      #     self.right_arm_joint_names_indices[joint.name()] = joint_index
      # elif "left" in joint.name():
      #     self.left_arm_joint_names_indices[joint.name()] = joint_index
    

    # print("ALL THE RIGHT JOINT NAMES")
    # print(right_arm_joint_names_indices)
    # print("ALL THE LEFT JOINT NAMES")
    # print(left_arm_joint_names_indices)
    # Sanity check - 
    # print('joint position lower limit: ', joint.position_lower_limits())
