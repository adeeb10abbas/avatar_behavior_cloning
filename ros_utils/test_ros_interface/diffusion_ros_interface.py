...
class DiffusionROSInterface:
    ...
    def publish_actions(self, parsed_action_dict: dict):
        """
        ...
        left_gripper_action = self.interpolate_action(parsed_action_dict["left_gripper_action"], action_publish_rate)
        # print("interpolated len of left gripper action: ", len(left_gripper_action))
        right_gripper_action = self.interpolate_action(parsed_action_dict["right_gripper_action"], action_publish_rate)
        left_arm_action = self.interpolate_action(parsed_action_dict["left_arm_action"], action_publish_rate)
        right_arm_action = self.interpolate_action(parsed_action_dict["right_arm_action"], action_publish_rate)

        rospy.loginfo(f"Publishing one action")
        t = time.monotonic()
        left_gripper_packet = create_RDDAPacket(left_gripper_action)
        right_gripper_packet = create_RDDAPacket(right_gripper_action)
        left_arm_packet = create_PTIPacket(left_arm_action)
        right_arm_packet = create_PTIPacket(right_arm_action)
        ...
        if (1.0/action_publish_rate - elapsed) > 0:
            ...
            rospy.loginfo("Publishing time exceeds the time budget")
        
        rospy.loginfo("All actions published successfully!")

    def parse_tensor_actions(self, action: np.ndarray) -> dict:
        """
        ...
        rospy.loginfo(f"Action shape: {action.shape}")
        ...
        print("Left gripper action shape: ", left_gripper_action.shape)
        parsed_action_dict = {
    ...
    def main(self):
        ...
        for i in range(self.policy.n_obs_steps):
            obs = self.get_obs()
        ## not sure why we need to do it for n_obs_steps times?
        ...
        try:
            ...
            while True:
                ...
                with torch.no_grad():
                    ...
                    obs_dict_np = get_real_obs_dict(env_obs=obs, shape_meta=self.cfg.task.shape_meta)
                    obs_dict = dict_apply(obs_dict_np, lambda x: torch.from_numpy(x).unsqueeze(0).to(self.device))
                    # rospy.loginfo("Inference: DO WE GET HERE?")
                    # rospy.loginfo(obs_dict['left_arm_pose'].shape)
                    s = time.monotonic()
                    result = self.policy.predict_action(obs_dict)
                    action = result["action"][0].detach().to("cpu").numpy()
                    ...
                    curr_time =  obs["timestamp"][0] #1723417020
                    ...
                    # print(action_commands.shape)
                    rospy.loginfo("action_timestamps: {}".format(action_timestamps))
                    rospy.loginfo("ACTION COMMAND SHAPE Inference: {}".format(action_commands.shape))

                    for i in range(4):
                        print(f"Action {i}: {action_commands[i]}")
                        action_comm = action_commands[i]
                        parsed_action_dict = self.parse_tensor_actions(action_comm)
                        # print("Action tuple shapes: ",parsed_action_dict["left_gripper_action"].shape, action_tuple[1].shape, action_tuple[2].shape, action_tuple[3].shape)
                        ...
                        self.publish_actions(parsed_action_dict)
